...
namespace Ki_WAT
{
    public class VEPBenchTransmissionZone : IVEPBenchZone
    {
        ...
        public void FromRegisters(ushort[] registers)
        {
            ...
            Console.WriteLine(ExchStatus.ToString() + " == " + registers[Offset_ExchStatus].ToString());

            if (ExchStatus != registers[Offset_ExchStatus]) 
            {
                ExchStatus = registers[Offset_ExchStatus]; 
                changed = true; 
            }
